...
/* ── Mobile Card ── */
const MobileCard = ({ r, onView, onImageClick }) => {
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-3 mb-2 shadow-sm">
            <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2.5">
                    <button
                        type="button"
                        onClick={() => r.vehicleImg && onImageClick({ url: r.vehicleImg, label: r.vehicleNumber })}
                        className={`w-11 h-11 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden flex-shrink-0 ${r.vehicleImg ? "cursor-zoom-in" : "cursor-default"}`}
                        title={r.vehicleImg ? "View vehicle photo" : ""}
                    >
                        {r.vehicleImg
                            ? <img src={r.vehicleImg} className="w-full h-full object-cover" alt="" />
                            : <div className="w-full h-full flex items-center justify-center text-slate-300 text-[9px] font-bold">No Img</div>
                        }
                    </button>
                    <div>
                        <span className="text-[10px] text-blue-600 block">{date.toLocaleDateString("en-GB")}</span>
                        <span className="text-[10px] bg-green-50 text-green-700 border border-green-200 rounded-lg px-1.5 py-0.5 font-mono font-bold text-slate-700 mt-0.5 inline-block">{r.tripNumber}</span>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-1 mb-2">
                {[
                    ...
                    <div key={label}>
                        ...
                        <p className={`text-xs text-slate-700 font-semibold ${upper ? "uppercase" : ""}`}>{value || "—"}</p>
                    </div>
                ))}
            </div>
...
const CarRentPage = () => {
    ...
    const rentalRows = rentals.map(r => ({
        ...
        vehicleNumber: r.vehicleNumber, vehicleImg: r.vehicleImg ?? null,
            ...
            <div className="flex-1 overflow-hidden">
                {loading ? (
                    ...
                    <div className="h-full overflow-y-auto p-2">
                        ...
                        {filteredRows.map((r, i) => <MobileCard key={i} r={r} onView={openRental} onImageClick={setLightbox} />)}
                    </div>
                ) : (
                            ...
                            <table className="w-full border-collapse" style={{ minWidth: "700px" }}>
                                <thead className="sticky top-0 z-20">
                                    ...
                                    <tr className="bg-slate-50 border-b-2 border-slate-200">
                                        ...
                                        <th className="p-1 border-r border-slate-200"><MultiSelectFilter options={getOptionsFor("tripNote")} selected={noteFilter} onChange={setNoteFilter} /></th>
                                        <th className="p-1" />
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        return (
                                            <tr key={i} className="border-b border-slate-100 hover:bg-amber-50/30 even:bg-slate-50/40 transition-colors text-center text-[12px]">
                                                ...
                                                <td className="px-2.5 py-2">
                                                    <span className="text-[10px] bg-green-50 text-green-700 border border-green-200 rounded-lg px-1.5 py-0.5 font-mono font-bold">{r.tripNumber}</span>
                                                </td>
                                                <td className="px-2.5 py-2 text-slate-800 font-mono font-bold max-w-[120px] truncate text-left">{r.vendorName}</td>
                                                <td className="px-2.5 py-2 text-slate-800 font-mono font-bold max-w-[120px] truncate text-left">{r.driverName}</td>
                                                <td className="px-2.5 py-2 text-slate-700 uppercase font-mono font-semibold">{r.vehicleNumber}</td>
                                                <td className="px-2.5 py-2">
                                                    <div className="flex justify-center">
                                                        <button
                                                            type="button"
                                                            onClick={() => r.vehicleImg && setLightbox({ url: r.vehicleImg, label: r.vehicleNumber })}
                                                            className={`w-9 h-9 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden flex-shrink-0 ${r.vehicleImg ? "cursor-zoom-in" : "cursor-default"}`}
                                                            title={r.vehicleImg ? "View vehicle photo" : "No photo"}
                                                        >
                                                            {r.vehicleImg
                                                                ? <img src={r.vehicleImg} className="w-full h-full object-cover" alt="" />
                                                ...
                                                <td className="px-2.5 py-2 text-left max-w-[160px]">
                                                    {r.tripNote && r.tripNote.trim()
                                                        ? <span className="text-[11px] font-bold py-1 rounded px-1 text-black bg-amber-200 truncate block" title={r.tripNote}>📝 {r.tripNote}</span>
