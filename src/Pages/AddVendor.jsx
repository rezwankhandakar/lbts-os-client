...
const AllVendor = () => {
  const axiosSecure = useAxiosSecure();
  // useRole থেকেই role + vendorName নাও — আলাদা API call দরকার নেই
  ...
  // role load হওয়ার পরেই fetch করো
  useEffect(() => {
    // role এখনো undefined মানে useRole query pending — wait করো
  ...
  const handleUpdate = async (e) => {
    ...
    if (res.data.modifiedCount > 0) {
      ...
      setEditVendor(null);
      setUpdatedImg("");
      setVendors(prev => prev.map(v => v._id === editVendor._id ? { ...v, ...updatedData } : v));
    }
  };

  // role undefined মানে এখনো loading
  if (role === undefined || loading) return <LoadingSpinner text="Fetching Records..." />;
  ...
  return (
    <div className="p-6 md:p-10 bg-[#F1F5F9] min-h-screen font-sans">
      ...
      {vendors.length === 0 ? (
        <div className="max-w-7xl mx-auto bg-white rounded-xl border border-slate-200 shadow-sm p-16 text-center">
          <p className="text-slate-400 italic text-sm">No vendor data found.</p>
          {isVendorRole && (
            <p className="text-slate-400 text-xs mt-2">Admin আপনাকে কোনো vendor এর সাথে link করেনি।</p>
          )}
