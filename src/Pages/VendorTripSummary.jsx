    ...
    <div ref={ref} className="relative w-full">
      ...
      {open && (
        <div className="fixed bg-white border border-gray-200 rounded-lg shadow-xl min-w-[140px] w-max max-w-[220px] overflow-hidden"
          ...
          {options.length > 5 && <div className="p-1.5 border-b border-gray-100"><input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" className="w-full px-2 py-1 text-xs border border-gray-200 rounded outline-none" /></div>}
          <div className="max-h-40 overflow-y-auto">
            {filtered.length === 0 ? <div className="px-3 py-2 text-xs text-gray-400 text-center">No results</div>
              : filtered.map(opt => (
          ...
          {selected.length > 0 && <div className="border-t border-gray-100 p-1"><button onClick={() => onChange([])} className="w-full text-[10px] text-gray-400 uppercase tracking-widest py-0.5 hover:text-gray-700">Clear all</button></div>}
        </div>
      )}
    </div>
  );
...
const TripCard = ({ t, idx, ps, onView }) => {
  ...
  const payBadge = () => {
    ...
    if (ps.status === "paid")    return <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200">✓ Paid</span>;
    if (ps.status === "partial") return <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200">Partial</span>;
    return <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-600 border border-red-200">Unpaid</span>;
  };
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-3.5 py-2.5 bg-gray-50 border-b border-gray-100">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[10px] text-gray-400">{new Date(t.createdAt).toLocaleDateString("en-GB")}</span>
          <button onClick={() => onView(t)} className="px-2.5 py-1 bg-gray-800 hover:bg-gray-700 text-white text-[10px] font-semibold rounded transition">View</button>
        </div>
      </div>
      <div className="px-3.5 py-2.5">
        <div className="flex items-center justify-between gap-2 mb-2">
          <div><p className="text-[8px] text-gray-400 uppercase font-bold tracking-widest">Driver</p><p className="text-xs font-semibold text-gray-800">{t.driverName}</p></div>
          <div className="text-right"><p className="text-[8px] text-gray-400 uppercase font-bold tracking-widest">Vehicle</p><p className="text-xs font-semibold text-gray-700 uppercase">{t.vehicleNumber}</p></div>
        </div>
        <div className="grid grid-cols-4 gap-1.5 pt-2 border-t border-gray-100">
          <div className="text-center">
            <p className="text-[8px] text-gray-400 uppercase font-bold tracking-wider">Rent</p>
            {t.rent != null ? <p className="text-[11px] font-bold text-green-700">৳{Number(t.rent).toLocaleString()}</p>
              : <p className="text-[9px] text-red-400 font-bold mt-0.5">—</p>}
          </div>
          <div className="text-center">
            <p className="text-[8px] text-gray-400 uppercase font-bold tracking-wider">Lebor</p>
            {t.leborBill != null ? <p className="text-[11px] font-bold text-green-700">৳{Number(t.leborBill).toLocaleString()}</p>
              : <p className="text-[9px] text-orange-400 font-bold mt-0.5">—</p>}
          </div>
          <div className="text-center">
            <p className="text-[8px] text-gray-400 uppercase font-bold tracking-wider">Advance</p>
            <p className="text-[11px] font-bold text-orange-600">{t.advance != null ? `৳${Number(t.advance).toLocaleString()}` : "—"}</p>
          </div>
          <div className="text-center bg-indigo-50 rounded-lg px-1 py-0.5">
            <p className="text-[8px] text-indigo-400 uppercase font-bold tracking-wider">Total</p>
            <p className="text-[11px] font-black text-indigo-700">{t.rent != null || t.leborBill != null ? `৳${tripTotal.toLocaleString()}` : "—"}</p>
          </div>
        </div>
      </div>
    </div>
...
const VendorTripSummary = () => {
  ...
  useEffect(() => {
    axiosSecure.get(`/vendors/${id}`).then(r => setVendor(r.data)).catch(console.error).finally(() => setVendorLoading(false));
  }, [id]);
  ...
  useEffect(() => {
    ...
    axiosSecure.get(`/car-rents?month=${tripMonth}&year=${tripYear}&page=1&limit=5000`)
      ...
      .catch(console.error).finally(() => setTripLoading(false));
  }, [tripMonth, tripYear, vendor]);
  ...
  const rowMatch = (t, ex = null) => {
    ...
    return chk("tripNumber", tripFilter, t.tripNumber) && chk("driverName", driverFilter, t.driverName) && chk("vehicleNumber", vehicleFilter, t.vehicleNumber)
  ...
  const getOpts = (field) => {
    const map = new Map();
    trips.forEach(t => { if (!rowMatch(t, field)) return; const v = t[field]?.toString().trim(); if (v && !map.has(v.toLowerCase())) map.set(v.toLowerCase(), v); });
    return Array.from(map.values()).sort((a, b) => a.localeCompare(b));
  };
  ...
  const handleReset = () => {
    ...
    setTripFilter([]); setDriverFilter([]); setVehicleFilter([]); setDateFilter(""); setRentFilter(""); setLeborBillFilter("");
  ...
  const handleExport = async () => {
    const { value: et } = await Swal.fire({
      title: "Export to Excel",
      html: `<div style="text-align:left;padding:8px 0">
        <label style="display:flex;align-items:center;gap:8px;margin-bottom:10px;cursor:pointer;font-size:13px"><input type="radio" name="et" value="filtered" checked style="accent-color:#374151"><span><b>Filtered</b> (${filteredTrips.length} rows)</span></label>
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px"><input type="radio" name="et" value="full"><span><b>Full month</b> — ${MONTHS[tripMonth - 1]} ${tripYear}</span></label>
      </div>`,
    ...
    try {
      ...
      const rows = src.map((t, i) => ({ "#": i + 1, Date: new Date(t.createdAt).toLocaleDateString("en-GB"), Trip: t.tripNumber, Driver: t.driverName, Vehicle: t.vehicleNumber, Point: t.challans ? t.challans.filter(c => !c.isReturn).length : t.totalChallan, "Rent": t.rent != null ? Number(t.rent) : "", "Lebor": t.leborBill != null ? Number(t.leborBill) : "", "Advance": t.advance != null ? Number(t.advance) : "", "Total": (t.rent != null ? Number(t.rent) : 0) + (t.leborBill != null ? Number(t.leborBill) : 0) }));
      const ws = XLSX.utils.json_to_sheet(rows); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Trips");
      saveAs(new Blob([XLSX.write(wb, { bookType: "xlsx", type: "array" })], { type: "application/octet-stream" }), `${vendor?.vendorName}_${et === "filtered" ? "Filtered" : "Full"}_${tripMonth}_${tripYear}.xlsx`);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 pb-3 space-y-2">
        {/* Row 1 */}
        <div className="flex items-center gap-2">
          <button onClick={() => navigate(-1)} className="p-2 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 text-slate-500 transition shrink-0">
            <ArrowLeft size={15} />
          </button>
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-2 shadow-sm flex-1 min-w-0">
            <div className="w-7 h-7 rounded-md bg-slate-800 overflow-hidden flex items-center justify-center shrink-0">
              {vendor.vendorImg ? <img src={vendor.vendorImg} className="w-full h-full object-cover" alt="" /> : <Briefcase size={13} className="text-white" />}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-black text-slate-800 truncate">{vendor.vendorName}</p>
              <p className="text-[9px] text-slate-400">Trip Summary · <span className="font-bold">{filteredTrips.length}/{trips.length}</span> trips</p>
            </div>
          </div>
        </div>
        {/* Row 2: controls */}
        <div className="flex items-center gap-1.5 flex-wrap">
          <select className="border border-gray-200 px-2 py-1.5 rounded text-xs bg-white text-gray-700 focus:outline-none flex-1 min-w-[100px] max-w-[140px]"
            value={tripMonth} onChange={e => setTripMonth(parseInt(e.target.value))}>
            {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
          </select>
          <input type="number" className="border border-gray-200 px-2 py-1.5 rounded text-xs bg-white text-gray-700 w-16 focus:outline-none"
          ...
          <button onClick={handleReset} className="flex items-center gap-1 px-2.5 py-1.5 text-xs rounded border border-red-200 text-red-500 hover:bg-red-500 hover:text-white transition">
          ...
          <button onClick={handleExport} className="flex items-center gap-1 px-2.5 py-1.5 text-xs rounded bg-gray-800 text-white hover:bg-gray-700 transition ml-auto">
        ...
        {showFilters && (
          <div className="p-3 bg-white border border-gray-200 rounded-xl shadow-sm">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Date</p>
                <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)} className="w-full px-2 py-1 border border-gray-200 rounded text-[10px] outline-none bg-white" /></div>
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Trip</p><MultiSelectFilter options={getOpts("tripNumber")} selected={tripFilter} onChange={setTripFilter} placeholder="All" /></div>
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Driver</p><MultiSelectFilter options={getOpts("driverName")} selected={driverFilter} onChange={setDriverFilter} placeholder="All" /></div>
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Vehicle</p><MultiSelectFilter options={getOpts("vehicleNumber")} selected={vehicleFilter} onChange={setVehicleFilter} placeholder="All" /></div>
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Rent</p>
                <SimpleSelect value={rentFilter} onChange={setRentFilter} options={[{value:"",label:"All"},{value:"added",label:"✓ Added"},{value:"missing",label:"✗ Missing"}]} /></div>
              <div><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1">Lebor Bill</p>
                <SimpleSelect value={leborBillFilter} onChange={setLeborBillFilter} options={[{value:"",label:"All"},{value:"added",label:"✓ Added"},{value:"missing",label:"✗ Missing"}]} /></div>
            </div>
          </div>
        )}
      </div>
      ...
      {filteredTrips.length > 0 && (
        <div className="flex-shrink-0 bg-slate-800 rounded-xl px-4 py-3 mb-3">
          {/* Mobile: 3x2 grid */}
          <div className="grid grid-cols-3 gap-x-3 gap-y-2 sm:hidden">
          ...
          <div className="hidden sm:flex flex-wrap items-center gap-x-4 gap-y-2">
            <div className="flex items-center gap-3">
              {summaryItems.slice(0, 3).map((item, i, arr) => (
                <React.Fragment key={i}>
                  <div><p className="text-[8px] text-slate-400 uppercase font-black tracking-widest leading-none mb-0.5">{item.label}</p><p className={`text-sm font-black ${item.color}`}>{item.value}</p></div>
      ...
      <div className="flex-1 min-h-0 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
        <div className="flex-shrink-0 px-4 py-2.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center gap-2"><ReceiptText size={14} className="text-slate-600" /><span className="text-xs font-black text-slate-700 uppercase tracking-widest">Trips</span></div>
        ...
        {tripLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-2 opacity-30"><Truck size={36} /><p className="text-xs font-black uppercase tracking-widest italic">No trips found</p></div>
        ) : (
          <>
            {/* ── MOBILE: card list ── */}
              ...
              <div className="p-3 space-y-2.5">
                ...
                <div className="bg-slate-800 rounded-xl px-4 py-3 grid grid-cols-4 gap-2 text-center">
                  {[
                    ...
                    <div key={i}><p className="text-[8px] text-slate-400 uppercase font-black tracking-widest leading-none mb-0.5">{item.l}</p><p className={`text-[10px] font-black ${item.c}`}>{item.v}</p></div>
                  ))}
                </div>
              </div>
            </div>
              ...
              <table className="w-full border-collapse text-xs" style={{ minWidth: "680px" }}>
                <thead className="sticky top-0 z-20">
                  ...
                  <tr className="bg-gray-50 border-b-2 border-gray-200">
                    ...
                    <th className="px-1 py-1 border-r border-gray-100" style={{minWidth:"88px"}}><MultiSelectFilter options={getOpts("tripNumber")} selected={tripFilter} onChange={setTripFilter} placeholder="All"/></th>
                    <th className="px-1 py-1 border-r border-gray-100" style={{minWidth:"96px"}}><MultiSelectFilter options={getOpts("driverName")} selected={driverFilter} onChange={setDriverFilter} placeholder="All"/></th>
                    <th className="px-1 py-1 border-r border-gray-100" style={{minWidth:"88px"}}><MultiSelectFilter options={getOpts("vehicleNumber")} selected={vehicleFilter} onChange={setVehicleFilter} placeholder="All"/></th>
                ...
                <tbody>
                  {filteredTrips.map((t, idx) => {
                    const tripTotal = (t.rent!=null?Number(t.rent):0)+(t.leborBill!=null?Number(t.leborBill):0);
                    const ps=payStatus[t._id]; const bill=Number(t.rent||0)+Number(t.leborBill||0);
                    return (
                      <tr key={t._id} className="border-b border-gray-100 hover:bg-amber-50 even:bg-gray-50/40 transition-colors">
                        ...
                        <td className="px-3 py-2 text-center">{!ps||bill===0?<span className="text-gray-300 text-[10px]">—</span>:ps.status==="paid"?<span className="text-[9px] font-semibold px-2 py-0.5 rounded border text-emerald-700 bg-emerald-50 border-emerald-200 uppercase whitespace-nowrap">✓ Paid</span>:ps.status==="partial"?<span className="text-[9px] font-semibold px-2 py-0.5 rounded border text-amber-700 bg-amber-50 border-amber-200 uppercase whitespace-nowrap">Partial</span>:<span className="text-[9px] font-semibold px-2 py-0.5 rounded border text-red-600 bg-red-50 border-red-200 uppercase whitespace-nowrap">Unpaid</span>}</td>
      ...
      <CarRentDetailsModal selectedRental={selectedRental} setSelectedRental={setSelectedRental} onRentalUpdate={handleRentalUpdate} readOnly={true} />
    </div>
  );
};
