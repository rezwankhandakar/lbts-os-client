...
//   vendor:      { label: 'Vendor',      bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-100'  },
...
//                             </button>
//                             <button onClick={() => handleRole(user._id, 'vendor')} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-amber-50 hover:text-amber-700 transition-colors border-b border-gray-100">
//                               <UserPlus size={15} /> Make Vendor
//                             </button>
...
const AssignVendorModal = ({ open, onClose, user, vendors, onAssign }) => {
  ...
  useEffect(() => {
    if (open) {
      setSelectedVendor(user?.vendorName || '');
      setSearch('');
    }
  }, [open, user]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{user.displayName} — {user.email}</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700"><X size={18} /></button>
        </div>
        <div className="p-5 space-y-3">
          <p className="text-xs text-gray-500">এই user কোন vendor এর সাথে link করবে সেটা select করো। সে শুধু সেই vendor এর data দেখতে পাবে।</p>
          <input
          ...
          <div className="max-h-48 overflow-y-auto space-y-1 border border-gray-100 rounded-lg p-1">
            {filtered.length === 0 ? (
              ...
              >
                ...
                {selectedVendor === v.vendorName && <Check size={14} />}
              </button>
            ))}
          </div>
          {selectedVendor && (
        ...
        <div className="flex gap-3 px-5 pb-5">
          ...
          <button onClick={handleAssign} disabled={saving || !selectedVendor}
            ...
            {saving ? 'Saving…' : 'Assign & Make Vendor'}
          </button>
        </div>
      </div>
    </div>
...
const UserManagement = () => {
  ...
  // vendors list for assignment modal
  ...
  // vendor assign করলে role + vendorName একসাথে পাঠাও
  const handleAssignVendor = async (id, vendorName) => {
    try {
      ...
      Swal.fire('Error!', 'Vendor assignment failed', 'error');
    }
  };
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Role</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider">Vendor Link</th>
                  <th className="px-6 py-3.5 text-xs font-semibold text-gray-600 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                  ...
                  return (
                    <tr key={user._id} className={`hover:bg-gray-50/50 transition-colors ${isActionLoading ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-right relative" ref={openDropdownId === user._id ? dropdownRef : null}>
                        ...
                        {openDropdownId === user._id && (
                          <div className="absolute right-6 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-xl z-50 py-1 overflow-hidden">
                            ...
                            {/* vendor — modal দিয়ে assign */}
                            <button
                              onClick={() => { setOpenDropdownId(null); setAssignVendorUser(user); }}
                              className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-teal-700 hover:bg-teal-50 transition-colors border-b border-gray-100"
                            >
                              <UserPlus size={15} /> Make Vendor 🏢
                            </button>
