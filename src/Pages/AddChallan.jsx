...
const AddChallan = () => {
  ...
  /* ── Shared input class ── */
  const inp = "input input-bordered w-full border-slate-200 focus:ring-2 focus:ring-blue-500/20 h-10 text-sm";
  const inpSm = "input input-sm input-bordered w-full bg-white h-9 text-sm";

  return (
    <div className="bg-[#F8FAFC] min-h-screen">

      {/* ── Mobile: Recent Challan toggle bar ── */}
      {recentChallan && (
        <div className="lg:hidden sticky top-0 z-30 bg-white border-b border-slate-200 px-3 py-2 flex items-center justify-between shadow-sm">
          <span className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
            <History size={14} className="text-emerald-600" /> Recent: <span className="font-bold text-slate-800 uppercase">{recentChallan.customerName}</span>
          </span>
          <button
            onClick={() => setShowRecent(v => !v)}
            className="text-xs px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 font-semibold border border-emerald-200"
          >
      ...
      {/* ── Mobile: Recent Challan collapsible panel ── */}
      ...
      <div className="max-w-7xl mx-auto p-3 sm:p-4 md:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6">
          ...
          <div className="lg:col-span-7 bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden">
            ...
            {/* Form header */}
            <div className="bg-slate-50 border-b border-slate-200 px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-2.5">
              <Package className="text-blue-600 shrink-0" size={20} />
              <h2 className="text-base sm:text-lg font-bold text-slate-800 tracking-tight">Create Delivery Challan</h2>
            </div>

            <form onSubmit={handleSubmit(onSubmit)} className="p-3 sm:p-5 space-y-4">

              {/* Row 1: Customer + Phone + Zone */}
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3">
                <div className="sm:col-span-5">
                  <label className="text-[10px] font-semibold text-slate-500 mb-1 block uppercase tracking-wider">Customer Name</label>
                  <div className="relative">
                    <User className="absolute left-3 top-2.5 text-slate-400" size={16} />
                    <input {...register("customerName", { required: true })} className={`${inp} pl-9`} placeholder="Customer Name" />
                  </div>
                </div>

                <div className="sm:col-span-4">
                  <label className="text-[10px] font-semibold text-slate-500 mb-1 block uppercase tracking-wider">Receiver Phone</label>
                  <div className="relative">
                    <Phone className="absolute left-3 top-2.5 text-slate-400" size={16} />
                    <input {...register("receiverNumber", { required: true })} className={`${inp} pl-9`} placeholder="017XXXXXXXX" />
                  </div>
                </div>

                <div className="sm:col-span-3 relative">
                  <label className="text-[10px] font-semibold text-slate-500 mb-1 block uppercase tracking-wider">Zone</label>
                  <div className="relative">
              ...
              {/* Row 2: Address */}
              <div>
                <label className="text-[10px] font-semibold text-slate-500 mb-1 block uppercase tracking-wider">Address</label>
                <div className="relative">
                  <MapPin className="absolute left-3 top-2.5 text-slate-400" size={16} />
                  <input {...register("address", { required: true })} className={`${inp} pl-9`} placeholder="Enter Delivery Address..." />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                <div className="relative">
                  <label className="text-[10px] font-semibold text-slate-500 mb-1 block uppercase tracking-wider">Thana</label>
                  <div className="relative">
              ...
              <div className="pt-3 border-t border-slate-100">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-blue-600 hover:bg-blue-50 rounded-lg transition-colors font-semibold"
                  >
                ...
                <div className="space-y-2">
                  {fields.map((item, index) => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 p-2 sm:p-3 bg-slate-50 border border-slate-200 rounded-xl items-center">
                      {/* Product Name */}
                      <div className="col-span-5 sm:col-span-4 relative">
                        <input
                          ...
                          placeholder="Product"
                        />
                        <AutoDropdown fieldKey={`product-${index}`} autoData={autoData} activeField={activeField} setActiveField={setActiveField} setFormValue={(v) => setValue(`products.${index}.productName`, v)} />
                      </div>
                      ...
                      {/* Qty — hidden label on mobile, visible inline */}
                      <div className="col-span-2 sm:col-span-1">
                        <input
                          type="number"
                          {...register(`products.${index}.quantity`, { required: true })}
                          className={`${inpSm} text-center`}
                          placeholder="Qty"
                        />
                      </div>
                      ...
                      <div className="col-span-12 sm:col-span-1 flex sm:justify-center">
                        <button
                          ...
                          className="flex items-center gap-1 text-xs text-slate-400 hover:text-red-500 transition-colors sm:p-1.5"
                        >
                          <Trash2 size={15} />
              ...
              <button className="btn btn-primary w-full gap-2 h-11 text-sm font-bold rounded-xl border-none bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20">
                <Send size={16} /> Submit Challan
              </button>
            </form>
          </div>

          {/* ================= RECENT CHALLAN (Right — desktop only) ================= */}
          <div className="hidden lg:block lg:col-span-5">
            <div className="bg-white shadow-sm border border-slate-200 rounded-2xl sticky top-6">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
                <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                  <History className="text-emerald-600" size={18} /> Recent Challan
                </h3>
                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded uppercase tracking-wider">Latest</span>
              </div>
              ...
              {recentChallan ? (
                ...
                <div className="py-16 text-center text-slate-400 text-sm italic">No recent activity found.</div>
              )}
            </div>
          </div>
...
/* ── Reusable Recent Challan Card ── */
const RecentChallanCard = ({ challan, onDelete, onEdit, compact = false }) => (
  <div className={compact ? "space-y-3" : "p-5 space-y-4"}>
    {/* Customer + Delete */}
    <div className="flex justify-between items-start">
      <div className="flex-1 min-w-0">
        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">Customer Name</label>
        <h4 className={`font-bold text-slate-900 uppercase leading-tight truncate ${compact ? "text-base" : "text-xl"}`}>
          {challan.customerName}
        </h4>
      </div>
      <button onClick={() => onDelete(challan._id)} className="p-1.5 text-slate-300 hover:text-red-500 transition-colors shrink-0 ml-2">
        <Trash2 size={16} />
      </button>
    </div>
    ...
    <div className="grid grid-cols-2 gap-2">
      <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-lg">
        <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Receiver</label>
        <p className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
          <Phone size={12} className="text-blue-500 shrink-0" /> {challan.receiverNumber}
        </p>
      </div>
      <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-lg">
        <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Zone</label>
        <p className="text-xs font-semibold text-slate-700 flex items-center gap-1.5 uppercase">
          <Globe size={12} className="text-blue-500 shrink-0" /> {challan.zone}
        </p>
      </div>
    </div>
    ...
    <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-lg">
      <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Delivery Address</label>
      <p className="text-xs font-medium text-slate-600 leading-relaxed flex items-start gap-1.5 mb-1.5">
        <MapPin size={13} className="text-red-400 mt-0.5 shrink-0" /> {challan.address}
      </p>
      <div className="flex flex-wrap gap-1.5 border-t border-slate-200 pt-1.5">
        <span className="text-[10px] font-bold text-slate-500 bg-slate-200/50 px-2 py-0.5 rounded uppercase">
          <span className="text-blue-400">Thana: </span>{challan.thana}
        </span>
        <span className="text-[10px] font-bold text-slate-500 bg-slate-200/50 px-2 py-0.5 rounded uppercase">
          <span className="text-blue-400">District: </span>{challan.district}
        </span>
      </div>
    </div>
    ...
    {/* Products */}
    <div>
      <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-2">Items</label>
      <div className="space-y-1.5">
        {challan.products?.map((p, i) => (
          <div key={i} className="flex justify-between items-center px-3 py-2 bg-white border border-slate-200 rounded-xl shadow-sm">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 rounded-md bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-[10px] shrink-0">{i + 1}</div>
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              <span className="text-sm font-bold text-blue-600">{p.quantity}</span>
              <button onClick={() => onEdit(p, i)} className="p-1 text-slate-300 hover:text-blue-600 transition-colors">
                <Edit3 size={14} />
              </button>
            </div>
          </div>
    ...
    <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-slate-400">
      <span className="text-[10px] font-semibold italic">By: {challan.currentUser || "Admin"}</span>
      <span className="flex items-center gap-1 text-[10px] font-medium">
        <Clock size={11} /> {new Date(challan.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
      </span>
    </div>
  </div>
);
