...
const Login = () => {
  ...
  const [verificationPending, setVerificationPending] = useState(null); // email if waiting for verification
  ...
  const handleSignin = async (data) => {
    ...
    try {
      ...
      // FIX #47 — Handle EMAIL_NOT_VERIFIED server response
  ...
  // FIX #47 — Resend verification email
  const handleResendVerification = async () => {
    const email = verificationPending || watch("email");
    const password = watch("password");
    if (!email || !password) {
      toast.error("Enter email and password to resend");
      return;
    }
    setLoading(true);
    try {
      // Need to sign-in first to get the user object
      ...
      console.error(err);
      toast.error("Could not send verification email. Check your credentials.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 relative">
      ...
      <div className="card w-full max-w-md bg-white shadow-xl rounded-2xl relative z-10">
        <div className="card-body p-8">
          ...
          {/* FIX #47 — Email verification pending banner */}
          {verificationPending && (
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm">
              ...
              <button
                ...
                className="text-xs text-amber-900 underline hover:text-amber-600"
                disabled={loading}
              >
