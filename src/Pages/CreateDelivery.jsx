        ...
        try {
            ...
            if (res.data.modifiedCount || res.data.success) {
                ...
                setDeliveryQueue(prev =>
                    prev.map(item =>
                        item._id === editingChallan._id ? editingChallan : item
                    )
                );
...
const handleConfirmDispatch = async () => {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] md: font-sans antialiased text-slate-900">
                    ...
                    <div className="p-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="vendorName"
                                value={deliveryInfo.vendorName}
                                onChange={handleDeliveryInfoChange}
                                placeholder="Vendor Ltd."
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:border-green-500 focus:bg-white"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="vendorNumber"
                                value={deliveryInfo.vendorNumber}
                                onChange={handleDeliveryInfoChange}
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="driverNumber"
                                value={deliveryInfo.driverNumber}
                                onChange={handleDeliveryInfoChange}
                        ...
                        <div className="p-2 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {deliveryQueue.length > 0 && (
                                    <button onClick={() => setDeliveryQueue([])} className="p-2 hover:bg-red-50 text-red-500 rounded-xl transition">
                                        <FaTrashAlt size={16} />
                                    </button>
                                )}
                            </div>
            ...
            {/* Modal components can be placed here or in a separate file */}
...
const InputGroup = ({ label, icon, isSearchable, suggestions, onSelect, onSearch, ...props }) => (
    <div className="relative group">
        <label className="text-[10px] font-black text-slate-400 uppercase ml-1 mb-1 block group-focus-within:text-green-600 transition-colors">
            {label}
        </label>
        <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-green-500 transition-colors">
                {icon}
            </span>
            <input
                {...props}
                onChange={isSearchable ? (e) => onSearch(e.target.value) : props.onChange}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-500/5 transition-all"
            />
        </div>
        {isSearchable && suggestions?.length > 0 && (
            <div className="absolute top-full left-0 w-full bg-white border border-slate-200 rounded-xl shadow-xl z-[100] mt-2 overflow-hidden">
                {suggestions.map((v, i) => (
                    <div key={i} onClick={() => onSelect(v)} className="p-3 hover:bg-green-50 cursor-pointer border-b last:border-0 transition-colors">
    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all group">
        ...
        <div className="p-4">
            ...
            <div className="bg-slate-50 rounded-lg p-2 border border-slate-100">
                ...
                {data.products?.length > 2 && <p className="text-[9px] text-center text-slate-400 mt-1 font-bold italic">+{data.products.length - 2} more items</p>}
            </div>
        </div>
    </div>
);
...
const QueueItem = ({ item, onRemove }) => (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm relative group hover:border-green-200 transition-all">
        <button onClick={() => onRemove(item._id)} className="absolute top-4 right-4 p-2 text-slate-300 hover:text-red-500 transition-colors">
            <FaTimes size={16} />
        </button>
