  ...
  useEffect(() => {
    if (open && challan && product) {
      setFormData({
        ...
        thana: challan.thana || "", district: challan.district || "",
    ...
    >
      ...
      >
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-4 bg-gradient-to-r from-emerald-700 to-green-600">
          <div>
            <p className="text-white font-black text-sm tracking-tight">Edit Challan</p>
            <p className="text-emerald-200 text-[11px] font-mono mt-0.5">
              ID: {challan?._id?.slice(-8).toUpperCase()}
            </p>
          </div>
          <button onClick={onClose}
            className="p-2 hover:bg-white/20 rounded-xl text-white/70 hover:text-white transition">
            <X size={16} />
          </button>
        </div>
