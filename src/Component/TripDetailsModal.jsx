...
// import { X, Truck, User, Package, PhoneForwarded, Plus, Trash2, Pencil, Check } from "lucide-react";
...
// /* ─── Edit Trip Info Modal (Vehicle/Driver/Vendor) ─── */
...
//             `/deliveries/${tripId}/challan/${challan.challanId}/product`,
//             { productName: p.productName, model: p.model, quantity: Number(p.quantity) || 1 }
//           );
//         } else {
//           await axiosSecure.patch(
//             `/deliveries/${tripId}/challan/${challan.challanId}/product/${p._id}`,
//             { productName: p.productName, model: p.model, quantity: Number(p.quantity) || 1 }
//           );
//         }
//       }
...
//   const [editingTripInfo, setEditingTripInfo] = useState(false); // ── নতুন
...
//   const handleChallanSaved = (updatedChallan) => {
//     syncTrip({
//       ...trip,
//       challans: trip.challans.map(c =>
...
//   // ── Trip info save ──
//   const handleTripInfoSaved = (updatedInfo) => {
//     syncTrip({ ...trip, ...updatedInfo });
//   };
...
//       {/* Edit Trip Info Sub-modal */}
//       {editingTripInfo && (
//         <EditTripInfoModal
//           trip={trip}
//           axiosSecure={axiosSecure}
//           onSave={handleTripInfoSaved}
//           onClose={() => setEditingTripInfo(false)}
//         />
//       )}

//       {/* Edit Challan Sub-modal */}
//       {editingChallan && (
//         <EditChallanCard
//           tripId={trip._id}
//           challan={editingChallan}
//           axiosSecure={axiosSecure}
//           onSave={handleChallanSaved}
...
const ReturnModal = ({ tripId, challan, onSave, onClose, axiosSecure }) => {
  ...
  const handleSave = async () => {
    if (activeReturns.length === 0)
      return Swal.fire({ icon: "warning", title: "No return items", text: "Please set at least one return quantity" });
    setSaving(true);
    try {
                ...
                return (
                  <div key={i} className={`group border rounded-2xl p-4 hover:shadow-md transition-all duration-200
                    ...
                    <div className="flex justify-between items-start gap-3">
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        {/* Status badges — normal card only */}
                        {!isReturnCard && (
                          <div className="flex gap-1.5">
                            <div className="relative">
                              <span
